...
public class OSetImpl_ApplicationInstanceComponentData_Type_
  implements OSet_ApplicationInstanceComponentData_Type_ {
    protected java.util.Hashtable map;
    protected Set_ApplicationInstanceComponentData_ toplevel_set;
    static protected Type itemtype;


    public OSetImpl_ApplicationInstanceComponentData_Type_ (CallContext context) {
        if (itemtype == null) { itemtype = TypeManager.get(context, ApplicationInstanceComponentData.class); }
    }

    public OSetImpl_ApplicationInstanceComponentData_Type_ (CallContext context, java.util.Hashtable map) {
        this.map = map;
        if (itemtype == null) { itemtype = TypeManager.get(context, ApplicationInstanceComponentData.class); }
    }
    ...
    protected void initialise(CallContext context) {
        if (this.toplevel_set == null) {
            ...
            this.toplevel_set = new SetImpl_ApplicationInstanceComponentData_ (context);
    ...
    public Set_ApplicationInstanceComponentData_ tryGet  (CallContext context, Type index) {
        ...
        return (Set_ApplicationInstanceComponentData_) (map == null ? null : map.get(index));
    }
    ...
    private Set_ApplicationInstanceComponentData_ getEntry(CallContext context, Type index) {
        initialise(context);
        Set_ApplicationInstanceComponentData_ set = (Set_ApplicationInstanceComponentData_) map.get(index);
        if (set == null) {
            set = new SetImpl_ApplicationInstanceComponentData_ (context);
            map.put(index, set);
        }
        return set;
    }

    private void    _setSuper  (CallContext context, ApplicationInstanceComponentData item, Type index) {
    ...
    private void    _unsetSuper  (CallContext context, ApplicationInstanceComponentData item, Type index) {
        if (index.equals(itemtype)) return;
        // Iterator_Type_ supertypes = index.getSuperTypes(context).getNavigator(context);
        Iterator_Type_ supertypes = index.getAllSuperInterfaces(context).getNavigator(context);
        Type supertype;
